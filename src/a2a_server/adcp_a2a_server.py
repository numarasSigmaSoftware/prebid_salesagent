...
class AdCPRequestHandler(RequestHandler):
    ...
    async def _handle_update_media_buy_skill(self, parameters: dict, identity: ResolvedIdentity) -> dict:
        ...
        # shared request builder below. Any schema-invalid value therefore becomes
        # the same INVALID_REQUEST response regardless of how it reached A2A.
        with adcp_validation_boundary():
...
# (docs/building/by-layer/L0/a2a-guide.mdx § "AdCP Extension" — the unversioned
# prose path, which resolves at tag v3.1.1; dist/docs carries no 3.1.1 snapshot),
