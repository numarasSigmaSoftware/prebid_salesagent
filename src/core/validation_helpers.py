...
def suggest_validation_fix(validation_error: ValidationError) -> str:
    """Return AdCP 3.1.1's canonical validation recovery guidance.

    Field-specific diagnostics remain available in ``message``, ``field``, and
    ``details.validation_errors``.  The top-level suggestion is protocol
    metadata and must not drift by Pydantic error type or transport.
    """
    # Keep the parameter in the shared boundary API: callers provide the
    # ValidationError whose safe diagnostics are projected separately.
    del validation_error
    return VALIDATION_ERROR_SUGGESTION
