...
class CreativeAssignmentRepository:
    ...
    def unapproved_creative_ids(self, media_buy_id: str) -> list[str]:
        ...
        ``approved`` is the ONLY status that clears the gate. The pinned 3.1
        ``enums/creative-status.json`` is exactly ``processing, pending_review,
        approved, suspended, rejected, archived``, and that same six-value set
        repeats across create-media-buy-request, package-request,
        update-media-buy-request (``$defs.CreativeStatus``), get-task-status-response
        and the creative-status-changed webhook transition map. ``active`` is not
        among them — it is a MEDIA BUY status, and every ``status = "active"``
        producer in ``src/`` writes a media buy, an account, or an adapter's
        ``CheckMediaBuyStatusResponse``; there are zero creative producers. Reading
        it as cleared let a value no creative can hold satisfy the gate.
        ...
        return [c.creative_id for c in creatives if c.status != "approved"]
