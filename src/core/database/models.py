...
class PersistedMediaBuyStatus(StrEnum):
    ...
    READY = "ready"
    FAILED = "failed"
    # The execution claim's two states. ACTIVATING is written durably BEFORE the
    # adapter is dispatched, so exactly one concurrent approver may proceed and a
    # crash mid-dispatch leaves evidence that the ad server may already hold an
    # order. ACTIVATION_UNKNOWN records a dispatch whose outcome could not be
    # confirmed — the order may or may not exist, so it is neither active nor
    # failed. Both are "a state this seller keeps that the protocol has no word
    # for"; neither is seller-confirmed, so neither stamps confirmed_at.
