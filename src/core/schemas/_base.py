...
class GetMediaBuysResponse(ReplayableResponseMixin, NestedModelSerializerMixin, LibraryGetMediaBuysResponse):
    ...
    ``ReplayableResponseMixin`` is composed alongside the other idempotent read
    responses (see ``GetAdcpCapabilitiesResponse``): it adds a true-only
    ``replayed`` field so this response type carries the same replay signal as
    the rest of the AdCP 3.1.1 idempotency surface if a cached hit is ever
    served through it.
    """
