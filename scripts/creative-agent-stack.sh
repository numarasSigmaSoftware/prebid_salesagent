...
# the wrong agent after a bump). The un-keyed alias is what
# docker-compose.e2e.yml and the standalone `up` path run; _ensure_image
# repoints it at this pin before every run.
