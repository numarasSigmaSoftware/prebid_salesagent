...
def pinned_error_code_metadata() -> dict[str, dict[str, str]]:
    """Return ``{error_code: {recovery, suggestion}}`` from the pinned error-code enum.

    Sourced from the installed SDK's tree like every other consumer of this
    module — there is exactly one upstream pin (see
    ``tests/unit/test_pinned_schema_single_source.py``). That matters here more
    than elsewhere: ``AUTH_MISSING`` and ``AUTH_INVALID`` exist ONLY in the
    SDK's current enum, so the auth-split contract this metadata grades is
    unreadable from the older vendored fixture.

    The two sources' ``suggestion`` text diverges on 4 codes
    (``MEDIA_BUY_NOT_FOUND``, ``PACKAGE_NOT_FOUND``, ``REQUOTE_REQUIRED``,
    ``CREDENTIAL_IN_ARGS``) and their ``recovery`` on none. No caller of this
    function reads a divergent code — callers grade the auth split plus
    ``VALIDATION_ERROR``, all byte-identical across both sources — so the
    migration is inert for them. Consumers that grade suggestion text for the
    divergent codes stay on the vendored fixture; see
    ``docs/adcp-spec-version.md`` "Pinned schema sources" and #1883.
    """
    metadata = load("error-code.json").get("enumMetadata")
    if not isinstance(metadata, dict):
        raise PinnedSchemaError("Pinned error-code.json has no enumMetadata object")
    ...
    for code, entry in metadata.items():
        ...
        if not isinstance(entry, dict):
            raise PinnedSchemaError(f"Pinned error code {code!r} metadata is not an object")
        if not all(isinstance(key, str) and isinstance(value, str) for key, value in entry.items()):
            raise PinnedSchemaError(f"Pinned error code {code!r} metadata contains non-string fields")
        entries[code] = entry
    return entries
