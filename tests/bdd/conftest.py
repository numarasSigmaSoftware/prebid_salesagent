...
def pytest_collection_modifyitems(items: list[pytest.Item]) -> None:
    ...
    for item in items:
        ...
        # T-UC-003-boundary-revision is wired (see the `_UC003_REVISION` set in
        # `_harness_env`'s UC-003 branch, below), so its steps
...
def _harness_env(request: pytest.FixtureRequest, ctx: dict) -> Generator[None, None, None]:
    ...
    if uc == "UC-002":
        ...
        # The four revision scenarios: the two BR-RULE-215 invariants (INV-4
        # increments-and-returns, INV-6 revision-and-idempotency-independent) plus the
        # two Scenario Outlines that partition and bound the token
        # (T-UC-003-partition-revision, T-UC-003-boundary-revision). They were dormant
        # for a reason that was not "the harness cannot reach them": they had NO step
        # definitions at all, so the xfail below was standing in for missing work rather
        # than for a production gap. The steps exist now, and these grade the obligation
        # the whole revision surface rests on — a mutating update advances the buyer's
        ...
        # Wiring them makes their steps RUN; the rows that this seller cannot satisfy
        # until #1607/#1689 land are routed to strict xfail per ROW in
        # `pytest_collection_modifyitems` above, so a fix XPASSes instead of hiding.
        _UC003_REVISION = {
            "T-UC-003-revision-success-increments",
            "T-UC-003-revision-and-idempotency-independent",
        ...
        if (
            ...
            with _db_scope_for(request, e2e_config), MediaBuyDualEnv(e2e_config=e2e_config) as env:
                ...
                yield
        elif marker_names & (_UC003_MANUAL_APPROVAL_WIRED | _UC003_REVISION):
            ...
            pytest.xfail(
                ...
                "follow-up). NOTE: the `revision` rows in the generated feature encode the POST-#1689 "
                "contract (matches_current -> success, stale/ahead -> CONFLICT) and contradict shipped "
                "behavior, which rejects every schema-valid revision with UNSUPPORTED_FEATURE until "
                "optimistic-concurrency lands. They ARE wired: every row that sends a token is routed "
                "per ROW to strict xfail in pytest_collection_modifyitems, so each XPASSes on its own "
                "the moment #1607/#1689 ships. Graduation is gated on that work, not on harness work."
            )
