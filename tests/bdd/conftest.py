...
# The two BR-RULE-215 revision scenarios. They were dormant for a reason that was
# not "the harness cannot reach them": they had NO step definitions at all, so the
# xfail below was standing in for missing work rather than for a production gap.
...
# manual-approval arm, so they share it. Hoisted (not function-local) for the same
# reason as the two sibling UC-003 sets above: so _wired_scenario_tags() can see it.
_UC003_REVISION_WIRED: set[str] = {
...
ENV_ROUTES: list[EnvRoute] = [
    ...
    EnvRoute(
        ...
        when=_uc(
            ...
            lambda m: (
                ...
                # T-UC-003-local-required-idempotency-v311: required-key validation
                # on the update path needs the same full update flow as ext-
                # scenarios to reach the wire boundary that rejects a missing key.
    ...
    # ── UC-006 ──────────────────────────────────────────────────────────────
    EnvRoute(
        # The hand-authored v3.1.1 idempotency companion — CreativeSyncIdempotencyWireEnv
        # is a distinct env from the general uc006-creative-sync row below (kept out
        # of the generated adcp-req feature until that derivative source is
        # reconciled with the pinned authoritative spec). Must be tried BEFORE the
        # general row: first-match-wins, and a creative-idempotency-v311 scenario
        # always wants this dedicated env regardless of what other UC-006 tags it
        # also carries.
        tag="uc006-idempotency-wire",
    ...
    # ── UC-010 ──────────────────────────────────────────────────────────────
    # Deliberately NO "uc010-not-wired" catch-all row (unlike every other UC
    # above): UC-010 is unlike the rest — every currently-authored scenario is
    # supposed to be explicitly wired here, so a UC-010 scenario that reaches
    # _harness_env without matching this predicate raises loud instead of
    # xfailing (see the RuntimeError branch in _harness_env below). Adding a
    # catch-all row here would silently mask that signal as an ordinary xfail.
    EnvRoute(
        tag="uc010-capabilities",
        # Wired: the schema-derived VERSION_UNSUPPORTED scenarios, the
        # hand-authored resolution boundaries (#1546), and the capability-filter
        # companion. @context scenarios (BR-RULE-043 / POST-S9): the impl already
        # echoes req.context unchanged and the MCP/A2A wrappers + the REST GET
        # context= query param all forward it — CapabilitiesEnv grades the
        # round-trip on every transport.
        when=_uc(
            "UC-010",
...
def _harness_env(request: pytest.FixtureRequest, ctx: dict) -> Generator[None, None, None]:
    ...
    Every UC is a set of rows in ``ENV_ROUTES``, resolved through the one
    generic ``storyboard_spec.resolve_env_route`` / ``_run_env_route`` pair —
    a predicate row (``when``) is tried first, in declaration order, then a
    coarse ``uc``-bucket row. A UC recognized by ``storyboard_spec.detect_uc``
    but not (yet) claimed by any row xfails with a UC-specific reason (each UC
    keeps its own ``*-not-wired`` catch-all row carrying that reason) via the
    generic fallback below. UC-010 is the one exception: it deliberately has
    no ``*-not-wired`` row, so an unmatched UC-010 scenario raises loud instead
    (see the special-case below) — every currently-authored UC-010 scenario is
    supposed to be explicitly wired, so reaching this point means a new one was
    added without deliberately routing it.

    A tag ``storyboard_spec.detect_uc`` does not recognize at all falls through
    to the same fallback as ``uc=None`` — an opaque "No harness wired for
    None". That is a bug, not intended behavior: widen ``detect_uc`` instead of
    ...
    if route is None:
        if uc == "UC-010":
            # UC-010 is unlike every other UC: EVERY currently-authored scenario
            # is supposed to be explicitly wired via a row in ENV_ROUTES above.
            # Reaching here means a new UC-010 scenario was collected without
            # deliberately adding it to ENV_ROUTES — raise loud instead of
            # masking a wiring gap as an xfail.
            raise RuntimeError(
