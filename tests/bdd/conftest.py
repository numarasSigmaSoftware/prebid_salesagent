...
def pytest_collection_modifyitems(items: list[pytest.Item]) -> None:
    ...
    for item in items:
        ...
        # error "CONFLICT" with suggestion` — an assertion about the seller's real
        # response, NOT a StepDefinitionNotFoundError. This seller answers ANY
        # schema-valid `revision` with UNSUPPORTED_FEATURE and does NOT apply the update
        # (validate_update_media_buy_protocol_fields), because it does not implement
        # optimistic concurrency; emitting CONFLICT for a stale or ahead token instead is
        # what #1607 owns. strict=True so implementing it XPASSes and the graduation
        # workflow catches the row rather than letting it sit green-by-omission.
        #
        ...
        #
        # Two rows expect `success` while SENDING a revision (partition matches_current,
        # boundary "revision 1, buy at 1") and are the other half of #1607: the same
        # UNSUPPORTED_FEATURE answer that denies the CONFLICT rows their conflict also
        # denies these rows their success, so they fail for one cause, not two. They are
        # routed on the same `outcome` column, gated on the row actually SUPPLYING a
        # revision — the `<not provided>` rows carry outcome `success` too and genuinely
        # pass (no token, no rejection), so matching `success` alone would strict-xfail a
        # green row and turn it red by xpass. Reading the `value` column keeps that
        # distinction where the feature file states it.
        if marker_names & {"T-UC-003-boundary-revision", "T-UC-003-partition-revision"}:
            ...
            # `<not provided>` is the feature file's own sentinel for "omit the field".
            # A row that omits the token never reaches the UNSUPPORTED_FEATURE rejection,
            # so it must NOT be routed; if the sentinel is ever renamed, that row starts
            # matching and strict-xpasses loudly rather than going quietly unrouted.
            _row_sends_revision = str(_row.get("value") or "").strip() != "<not provided>"
            if "CONFLICT" in _row_outcome and is_a2a:
                # a2a used to fail EARLIER than the others and for a different reason:
                # _handle_update_media_buy_skill rebuilt the request from hand-listed
                # fields and `revision` was in none of them, so the token never reached
                # the tool (#1885). That drop is closed here — the skill handler now
                # preserves revision presence (adcp_a2a_server.py, `params["revision"]`
                # vs REVISION_OMITTED), and an a2a stale/ahead row reaches exactly the
                # same UNSUPPORTED_FEATURE answer as mcp/rest (probed at the wire:
                # "A2A boundary translating AdCPCapabilityNotSupportedError to envelope:
                # UNSUPPORTED_FEATURE"). The branch is kept only so a2a's routing stays
                # explicit; it now carries #1607's label like its siblings.
                item.add_marker(
                    pytest.mark.xfail(
                        reason=(
                            "cause=production-gap scope=per-transport ref=#1607 — a2a converges "
                            "with mcp/rest: the skill handler now forwards `revision`, so the "
                            "token reaches the tool and the seller answers a schema-valid "
                            "revision with UNSUPPORTED_FEATURE instead of the CONFLICT this row "
                            "expects. The earlier #1885 parameter drop is closed, so implementing "
                            "the optimistic-concurrency check DOES make this row gradeable. "
                            "#1259 owns the separate question of why no guard saw the drop."
                        ),
                        strict=True,
                    )
                )
                ...
                item.add_marker(
                    pytest.mark.xfail(
                        reason=(
                            "cause=production-gap scope=per-transport ref=#1607 — this seller "
                            "answers a schema-valid `revision` with UNSUPPORTED_FEATURE and does "
                            "not apply the update; the spec MUST instead reject a stale or ahead "
                            "token with CONFLICT. Steps execute and the token reaches the tool, so "
                            "the failure is the missing optimistic-concurrency check, not a "
                            "dropped parameter. scope=per-transport because each transport "
                            "enforces (or fails to enforce) independently, so each must xpass on "
                            "its own when #1607 lands."
                        ),
                        strict=True,
                    )
                )
            ...
            elif _row_outcome.strip() == "success" and _row_sends_revision:
                item.add_marker(
                    pytest.mark.xfail(
                        reason=(
                            "cause=production-gap scope=per-transport ref=#1607 — this row supplies "
                            "a revision that MATCHES the stored one and expects the update to "
                            "succeed, but this seller answers any schema-valid `revision` with "
                            "UNSUPPORTED_FEATURE and applies nothing "
                            "(validate_update_media_buy_protocol_fields, media_buy_update.py). "
                            "Accept-and-ignore is not an alternative reading: "
                            "update-media-buy-request.json states sellers MUST reject on mismatch "
                            "and MUST enforce the comparison atomically with the write, with no "
                            "MAY-ignore clause, so the token cannot be honoured until optimistic "
                            "concurrency lands — owned by #1607/#1689. Rows that omit the token "
                            "share this outcome and are deliberately not routed here. "
                            "scope=per-transport because each transport has to carry the token to "
                            "the tool and xpass on its own; a2a dropped it once (#1885)."
                        ),
                        strict=True,
                    )
                )

        # The two BR-RULE-215 revision invariants (INV-4 increments-and-returns, INV-6
        # revision-and-idempotency-independent) are plain Scenarios, not Outlines, so
        # there is no `outcome` column to route on — they are matched by tag. Both
        # demand a successful update while supplying a revision, which is the same
        # #1607/#1689 gap the `success` Examples rows above hit, so they carry the same
        # cause and the same strict flag.
        if marker_names & {
            ...
            item.add_marker(
                pytest.mark.xfail(
                    reason=(
                        "cause=production-gap scope=per-transport ref=#1607 — BR-RULE-215 INV-4 "
                        "and INV-6 require a mutating update to compare the supplied `revision`, "
                        "advance the stored value and report it. This seller instead answers any "
                        "schema-valid `revision` with UNSUPPORTED_FEATURE "
                        "(validate_update_media_buy_protocol_fields, media_buy_update.py), so no "
                        "success envelope is produced and neither the incremented revision nor "
                        "the independence of the idempotency check can be graded. Optimistic "
                        "concurrency is owned by #1607/#1689; strict=True so landing it XPASSes "
                        "here instead of leaving the invariant green-by-omission. "
                        "scope=per-transport for the same reason as the revision Examples rows "
                        "above: each transport must carry the token to the tool (a2a dropped it "
                        "once, #1885) and xpass on its own."
                    ),
                    strict=True,
                )
            )
...
def _build_uc010_unwired_env(e2e_config: object | None) -> AbstractContextManager:
    """The UC-010 catch-all: raise rather than xfail.

    Every other branch UC ends in an ``xfail_reason`` placeholder, because its
    unwired scenarios are genuinely dormant. UC-010 is the deliberate exception:
    its scenarios grade version negotiation, and a new one arriving without an
    explicit CapabilitiesEnv binding is a wiring omission, not a spec-production
    gap. Masking it as xfail would make the omission invisible.
    """
    raise RuntimeError(
        "A UC-010 scenario was collected without explicit CapabilitiesEnv wiring; "
        "bind it deliberately (add its tag to _UC010_VERSION_NEGOTIATION_WIRED / "
        "_UC010_CAPABILITY_FILTER_WIRED, or add an ENV_ROUTES row) instead of masking it with xfail"
    )
...
ENV_ROUTES: list[EnvRoute] = [
    ...
    EnvRoute(
        tag="uc003-ext",
        # _UC003_REQUIRED_IDEMPOTENCY_WIRED rides this row: the hand-authored
        # v3.1.1 required-key companion drives the UPDATE transport wrappers
        # against a seeded existing buy, exactly what this row already builds.
        when=_uc(
            "UC-003",
            lambda m: (
                any(t.startswith("T-UC-003-ext-") for t in m)
                or bool(m & _UC003_TARGETING_OVERLAY_TAGS)
                or bool(m & _UC003_REQUIRED_IDEMPOTENCY_WIRED)
            ),
        ),
    ...
    # ── UC-006 ──────────────────────────────────────────────────────────────
    # Declared BEFORE uc006-creative-sync: the hand-authored v3.1.1 key
    # companion needs the SUBCLASS env (genuine A2A Task framing, so the
    # failed-Task artifact envelope is captured), not the base CreativeSyncEnv
    # the broad generated suite uses.
    EnvRoute(
        tag="uc006-creative-idempotency-v311",
        when=_uc("UC-006", lambda m: "creative-idempotency-v311" in m),
        env_builder=_env("tests.harness.creative_sync.CreativeSyncIdempotencyWireEnv"),
    ),
    EnvRoute(
    ...
    # ── UC-010 ──────────────────────────────────────────────────────────────
    # Wired: the schema-derived VERSION_UNSUPPORTED scenarios, the hand-authored
    # capability-filter companion, and the @context round-trip scenarios
    # (BR-RULE-043 / POST-S9 — the impl echoes req.context unchanged and the
    # MCP/A2A wrappers plus the REST GET context= query param all forward it).
    # CapabilitiesEnv carries the negotiation envelope through each transport's
    # boundary validation.
    EnvRoute(
        tag="uc010-capabilities",
        when=_uc(
            "UC-010",
            lambda m: bool(m & (_UC010_VERSION_NEGOTIATION_WIRED | _UC010_CAPABILITY_FILTER_WIRED)) or "context" in m,
        ),
    ...
    EnvRoute(
        ...
        # Deliberately NOT an xfail_reason: an unbound UC-010 scenario must fail
        # loudly so it gets bound on purpose, rather than being masked as a
        # spec-production gap it is not.
        env_builder=_build_uc010_unwired_env,
    ),
    # ── UC-004 ──────────────────────────────────────────────────────────────
    EnvRoute(
        tag="uc004-create",
