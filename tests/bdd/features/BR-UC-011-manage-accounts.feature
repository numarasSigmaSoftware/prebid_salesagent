...
Feature: BR-UC-011 Manage Accounts
  ...
  # HAND-EDITED: grounded in spec artifacts (core/protocol-envelope.json,
  # account/sync-accounts-response.json) with no adcp-req requirement id, so
  # compile_bdd.py --merge would classify it LEGACY-DELETE and drop it. The marker
  # plus its HAND_MAINTAINED_SCENARIOS entry are what keep it alive across a re-render.
  @T-UC-011-sync-schema-valid @sync @schema @v3-1 @envelope @hand-edited
