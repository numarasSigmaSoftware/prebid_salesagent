...
def wire_error_dict(ctx: dict) -> dict:
    ...
    ``result.wire_capture_unavailable`` exempts one further case, and only that
    one: A2A's documented "direct raw" dispatch mode (an env whose ``*_raw()``
    wrapper raises, with no Task/Artifact framing to reconstruct wire from) has
    ...
    regression this guard exists to catch, so it takes the synthesized fallback
    instead of failing.
    """
    result = ctx.get("result")
    ...
    if envelope is None and transport not in (None, Transport.IMPL) and not wire_capture_unavailable:
