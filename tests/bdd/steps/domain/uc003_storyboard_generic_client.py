...
def when_update_media_buy_with_unknown_id(ctx: dict) -> None:
    ...
    Carries a fresh ``idempotency_key``: it is required on every mutating
    request, and validated BEFORE the media_buy lookup. Omitting it makes the
    seller answer VALIDATION_ERROR, so the scenario would grade the missing key
    rather than the unknown-id path it exists to grade.
    """
    ...
    payload = {
        ...
        "paused": True,
        "idempotency_key": fresh_idempotency_key("uc003-storyboard-unknown-id"),
...
def when_update_media_buy_recancel(ctx: dict) -> None:
    ...
    Carries a FRESH ``idempotency_key`` for the same reason as the sibling When
    step above. Fresh, not the first cancel's key: this is a distinct buyer
    request, and reusing the earlier key would make the seller replay the first
    cancel's cached success instead of deciding the re-cancel on its merits.
    """
    correlation_id = str(uuid4())
    ctx["correlation_id"] = correlation_id
    ...
    payload = {
        ...
        "canceled": True,
        "idempotency_key": fresh_idempotency_key("uc003-storyboard-recancel"),
        "context": {"correlation_id": correlation_id},
    }
    dispatch_via_client(ctx, "update_media_buy", payload)
