...
from tests.harness.account_sync import make_sync_accounts_request
...
def _sync_pre_create(ctx: dict, brand_domain: str, operator: str, billing: str, **extra: Any) -> None:
    ...
    entry.update(extra)
    req = make_sync_accounts_request(accounts=[entry])
...
def when_sync_accounts_with_table(ctx: dict, datatable: Any) -> None:
    ...
    try:
        req = make_sync_accounts_request(accounts=accounts)
...
def when_sync_empty_accounts(ctx: dict) -> None:
    ...
    try:
        req = make_sync_accounts_request(accounts=[])
        dispatch_request(ctx, req=req)
    except Exception as exc:
        ctx["error"] = exc
...
def when_sync_accounts_without_idempotency_key(ctx: dict) -> None:
    """Dispatch an actual omission through A2A, MCP, or REST wire framing."""
    from tests.harness._idempotency import OMIT_IDEMPOTENCY_KEY

    _setup_tenant_and_principal(ctx)
    dispatch_request(ctx, accounts=[], idempotency_key=OMIT_IDEMPOTENCY_KEY)
...
def when_sync_dryrun_and_delete_missing(ctx: dict, datatable: Any) -> None:
    ...
    rows = [dict(zip(headers, row, strict=True)) for row in datatable[1:]]
    accounts = _parse_sync_table(rows)
    req = make_sync_accounts_request(accounts=accounts, dry_run=True, delete_missing=True)
    dispatch_request(ctx, req=req)
...
def when_named_agent_sync_delete_missing(ctx: dict, name: str, datatable: Any) -> None:
    ...
    rows = [dict(zip(headers, row, strict=True)) for row in datatable[1:]]
    accounts = _parse_sync_table(rows)
    req = make_sync_accounts_request(accounts=accounts, delete_missing=True)
    dispatch_request(ctx, req=req, identity=identity)
...
def when_sync_brandless_entry(ctx: dict) -> None:
    ...
    req = make_sync_accounts_request(accounts=[{"account": {"account_id": "ref-001"}, "operator": "example.com"}])
    dispatch_request(ctx, req=req)
