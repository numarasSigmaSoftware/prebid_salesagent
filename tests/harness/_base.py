...
class BaseTestEnv:
    ...
    def _run_a2a_handler(
        ...
        **kwargs: Any,
        ...
        from tests.harness.transport import NO_IDENTITY_OVERRIDE, Transport, WireAuth
    ...
    def _run_mcp_client(
        ...
        **kwargs: Any,
        ...
        def _capture_result(result: Any) -> DeliverResult:
            """Build the DeliverResult for one MCP CallToolResult.

            ``raw_response`` carries the unprocessed FastMCP ``CallToolResult``
            through on the SAME return value (never a stashed attribute) — the
            authenticity check in
            ``tests/integration/test_harness_wire_response.py`` compares the
            captured wire dict against the object that actually crossed the
            in-memory MCP client boundary.
            """
            return DeliverResult(
    ...
    def _run_rest_request(self, endpoint: str, **kwargs: Any) -> Any:
        ...
        # Not an identity-omission sentinel (tests.harness.transport.NO_IDENTITY_OVERRIDE
        # is the canonical one for that, enforced by
        # test_architecture_harness_identity_sentinel.py) — this one just distinguishes
        # "no prior FastAPI dependency_overrides entry" from "prior entry was None".
        _absent_marker = object()
        previous_overrides = {
            dependency: app.dependency_overrides.get(dependency, _absent_marker) for dependency in auth_dependencies
        }
    ...
    def _configure_rest_auth_override(identity: Any) -> None:
        ...
        if isinstance(identity, WireAuth):
            # WireAuth removes the harness's dependency overrides for exactly this
            # request, so UnifiedAuthMiddleware and the real auth dependencies
            # consume the raw wire headers. _run_rest_request snapshots and
            # restores the previous overrides around the request.
