...
class BaseTestEnv:
    ...
    def call_via(self, transport: Transport, **kwargs: Any) -> TransportResult:
        ...
        # Per-call capture: clear the raw Task before dispatching so "absent" is
        # distinguishable from "left over". Without this reset a scenario that
        # dispatches twice leaves the FIRST call's Task visible, so a guard
        # asserting on Task state after a second dispatch that never produced one
        # would pass on the stale value.
