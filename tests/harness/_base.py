...
class BaseTestEnv:
    ...
    def __init__(
        ...
        **tenant_overrides: Any,
        ...
        # that actually crossed the in-memory MCP client boundary. The parsed
        # success wire itself rides DeliverResult.wire_response — this stash is
        # the raw CallToolResult object only, which has no return-value channel.
    ...
    def call_via(self, transport: Transport, **kwargs: Any) -> TransportResult:
        ...
        # Reset the raw MCP CallToolResult capture; _run_mcp_client sets it
        # fresh on success so McpDispatcher can surface the object that crossed
        # the in-memory client boundary.
    ...
    def _run_mcp_client(
        ...
        **kwargs: Any,
        ...
        def _capture_result(result: Any) -> DeliverResult:
            """Build the DeliverResult, stashing the raw CallToolResult alongside.

            The wire itself rides the RETURN VALUE (``DeliverResult.wire_response``)
            — never a per-env attribute. Only the raw ``CallToolResult`` object is
            stashed, because ``DeliverResult`` carries no channel for it and
            authenticity checks compare the captured wire dict against the object
