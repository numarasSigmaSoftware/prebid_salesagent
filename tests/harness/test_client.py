...
class TestClientA2ADispatchNoDb:
    ...
    def test_get_products_via_a2a_succeeds(self, monkeypatch):
        from contextlib import contextmanager
        from unittest.mock import MagicMock

        from tests.harness.product_unit import ProductEnv

        # AdCPRequestHandler.on_message_send unconditionally persists task state
        # via A2ATaskUoW before dispatching to the requested skill (real DB
        # write) — ProductEnv mocks only _get_products_impl's own dependencies,
        # not the A2A server's task-persistence machinery. Stub it out here so
        # this class lives up to its own "NoDb" name; the discovery-skill
        # dispatch path under test never reads the task row back.
        @contextmanager
        def _noop_uow(*_args, **_kwargs):
            uow = MagicMock()
            uow.tasks = MagicMock()
            yield uow

        monkeypatch.setattr("src.a2a_server.adcp_a2a_server.A2ATaskUoW", _noop_uow)
...
class TestRestRequestKwargsBodilessVerbs:
    ...
    The body still has to reach the server SOMEHOW for a bodiless verb — a
    GET dispatch (e.g. CapabilitiesEnv's ``protocols``/``context`` filters)
    sends it as ``params=`` (the query string) instead of ``json=``; dropping
    it outright would make query-param filtering unreachable through GET
    dispatch entirely. See ``tests/harness/test_harness_base.py::
    test_e2e_rest_dispatcher_forwards_wire_auth_and_uses_method_payload``
    for the end-to-end pin of the same contract.
    """

    def test_get_sends_body_as_params(self):
        from tests.harness.client import _rest_request_kwargs

        assert _rest_request_kwargs("get", {"some": "body"}) == {"params": {"some": "body"}}

    def test_delete_sends_body_as_params(self):
        from tests.harness.client import _rest_request_kwargs

        assert _rest_request_kwargs("delete", {"some": "body"}) == {"params": {"some": "body"}}
    ...
    def test_extra_kwargs_pass_through_regardless_of_verb(self):
        ...
        assert _rest_request_kwargs("get", {}, headers={"X": "1"}) == {"params": {}, "headers": {"X": "1"}}
...
class TestClientRestDispatchNoDb:
    ...
    def test_get_dispatch_does_not_pass_json_kwarg(self):
        ...
        # no `json` key (the TypeError this test guards against) — the body
        # travels as `params=` instead, per _rest_request_kwargs's contract.
        assert calls == [{"url": "/api/v1/capabilities", "params": {}}]
