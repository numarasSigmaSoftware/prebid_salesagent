...
def _rest_request_kwargs(method: str, body: dict[str, Any], **extra: Any) -> dict[str, Any]:
    ...
    Bodiless verbs (get/delete, see ``_BODILESS_REST_VERBS``) send *body* as
    ``params=`` (the query string) instead of ``json=`` — a JSON body kwarg is
    either rejected by the client (starlette TestClient.get/httpx.Client.get do
    not accept ``json=`` at all) or simply wrong to send, but the caller's flat
    kwargs (e.g. CapabilitiesEnv's ``protocols``/``context``) still need to
    reach the server somehow. Shared by in-process and e2e REST DELIVER so the
    rule is defined once, not per call site.
    """
    kwargs: dict[str, Any] = dict(extra)
    if method in _BODILESS_REST_VERBS:
        kwargs["params"] = body
    else:
        kwargs["json"] = body
    return kwargs
...
def e2e_identity_headers(identity: Any) -> dict[str, str]:
    ...
    simply omitted rather than sent empty. A ``WireAuth`` override supplies
    the raw wire headers UNCHANGED — no derivation, no auth-chain simulation —
    so token extraction and principal resolution stay production code under
    test (mirrors the in-process ``_run_mcp_client``/``_run_a2a_handler``/
    ``_run_rest_request`` WireAuth handling).
    """
...
def _unwrap_tool_success(
    ...
    return TransportResult(
        ...
        raw_response=delivered.raw_response,
...
def unwrap_a2a_error(exc: Exception, transport: Transport = Transport.A2A) -> TransportResult:
    ...
    raising — that attribute's PRESENCE (not its value) is the per-call signal
    for "this dispatch mode promised to capture wire". A direct-raw exception
    (e.g. an env whose ``deliver_a2a``/``call_a2a`` raises straight from a
    ``*_raw()`` wrapper, with no Task/Artifact framing to ever reconstruct
    wire from) never has the attribute at all — deliberately NOT covered by
    :func:`_wire_envelope_from_exception`'s synthesized fallback here, so
    ``wire_error_envelope`` stays honest: a raw-path env cannot masquerade its
    synthesized output as real wire (mirrors :func:`unwrap_mcp_error`'s
    real-wire-only rule). ``wire_capture_unavailable`` exposes that PRESENCE
    signal on the result so callers like
    ``tests/bdd/steps/_outcome_helpers.wire_error_dict`` can tell a legitimate
    no-wire dispatch apart from a stashing bug. See :func:`unwrap_mcp_error`
    for why this is one function rather than a copy per dispatch path.
    """
    ...
    return TransportResult(
        ...
        envelope={"transport": transport.value, "status": derive_error_status(wire)},
        wire_error_envelope=wire,
        # What production WOULD emit for the same exception — the only honest
        # error envelope for raw-path envs (e.g. CreativeSyncEnv); never a
        # substitute for the wire field above.
        synthesized_error_envelope=_envelope_from_adcp_error(exc),
