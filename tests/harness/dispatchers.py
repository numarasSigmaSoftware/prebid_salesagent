...
class A2ADispatcher:
    """Dispatch via ``env.deliver_a2a`` — the A2A transport wrapper.
    ...
    for the caught exception — see ``unwrap_a2a_error``'s docstring. This
    keeps ``wire_error_envelope`` honest — a raw-path env cannot masquerade
    its synthesized output as real wire.
    """
...
class McpDispatcher:
    ...
    def dispatch(self, env: BaseTestEnv, **kwargs: Any) -> TransportResult:
        ...
        # return value as the payload — see the A2A sibling above. raw_response
        # (the FastMCP CallToolResult) rides the same DeliverResult so
        # authenticity checks (tests/integration/test_harness_wire_response.py)
        # can compare the captured wire dict to the object that actually
        # crossed the in-memory MCP client boundary.
        return TransportResult(
            payload=delivered.payload,
...
class RestE2EDispatcher:
    ...
    def dispatch(self, env: BaseTestEnv, **kwargs: Any) -> TransportResult:
        ...
        # identity call unauthenticated instead. An explicit ``identity=None``
        # still means "send without auth headers" (no-auth test): it is not
        # the sentinel, so it flows straight through to
        # ``e2e_identity_headers``, which returns no headers for ``None`` —
        # the server's auth middleware then returns the real 401/structured
        # error. A ``WireAuth`` override is handled the same way, one level
        # down in ``e2e_identity_headers`` (shared by e2e REST/MCP/A2A).
