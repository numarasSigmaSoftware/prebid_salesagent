...
class TestBaseClassContract:
    ...
    def test_a2a_dispatcher_splits_stashed_wire_from_synthesized(self):
        ...
        Both doubles override ``deliver_a2a`` — the seam ``A2ADispatcher.dispatch``
        actually calls. Overriding ``call_a2a`` (the parsed-payload convenience
        that merely delegates to ``deliver_a2a``) would never execute, and the
        assertions below would grade ``BaseTestEnv.deliver_a2a``'s
        ``NotImplementedError`` instead of the split.
        """
