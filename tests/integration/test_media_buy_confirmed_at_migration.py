...
def _seed_pre_migration_media_buys(engine, tenant_id: str) -> None:
    ...
    # The operational job runs after the finalization-lease migration, so a
    # live finalizing buy must stay unconfirmed until it is actually published.
    _insert_media_buy(
        engine,
        f"{tenant_id}_finalizing",
        tenant_id=tenant_id,
        status="finalizing",
        approved_at=None,
        created_at=_CREATED,
...
def test_operational_backfill_uses_multiple_committed_batches(migration_db):
    ...
    # Four eligible rows prove the loop advances past a single-row batch and
    # confirm eligibility is case-insensitive like the model's canonical rule.
    commits: list[object] = []
    ...
    assert rows[f"{tenant_id}_finalizing"] is None
