...
# isolates ONE of the two consumers so a wiring mistake in either shows up here,
# not only downstream where an independent claim-lease mechanism (final_webhook_
# claimed_at) can mask a broken predicate for same-worker, same-run scenarios.
# ---------------------------------------------------------------------------
...
class TestHasSuccessfulFinal:
    ...
    def test_true_when_a_successful_final_is_logged(self, tenant_a, principal_a, media_buy_a):
        with get_db_session() as session:
            repo = DeliveryRepository(session, tenant_a)
            repo.create_log(
                log_id=str(uuid4()),
        ...
        with get_db_session() as session:
            repo = DeliveryRepository(session, tenant_a)
            assert repo.has_successful_final(media_buy_a) is True

    def test_false_when_the_only_success_is_not_final(self, tenant_a, principal_a, media_buy_a):
        """A successful periodic send must NOT count as the final.

        This is the discriminating case: a terminal buy commonly has prior
        successful "scheduled" sends from its serving days before its final is
        due. Collapsing the predicate to "any successful send exists" would
        report a final as already delivered when none was ever sent.
        """
        with get_db_session() as session:
            repo = DeliveryRepository(session, tenant_a)
            repo.create_log(
        ...
        with get_db_session() as session:
            repo = DeliveryRepository(session, tenant_a)
            assert repo.has_successful_final(media_buy_a) is False

    def test_false_when_the_only_final_failed(self, tenant_a, principal_a, media_buy_a):
...
class TestGetReportableForDeliveryFinalAntiJoin:
    ...
    Isolates the selection-time anti-join from the scheduler's claim-lease
    mechanism: these tests call the repository method directly, with no send
    attempted and no claim ever taken, so the anti-join is the only thing that
    can produce the observed inclusion/exclusion.
    """

    def _terminal_buy_with_webhook(self, tenant_id: str, principal_id: str, media_buy_id: str) -> None:
        with get_db_session() as session:
            session.add(
                MediaBuy(
                    media_buy_id=media_buy_id,
                    tenant_id=tenant_id,
                    principal_id=principal_id,
                    order_name="Terminal Order",
                    advertiser_name="Test Advertiser",
                    start_date=datetime(2026, 1, 1).date(),
                    end_date=datetime(2026, 1, 8).date(),
                    status="completed",
                    raw_request={"reporting_webhook": {"url": "https://buyer.example.com/webhook"}},
                )
            )
            session.commit()

    def test_selected_when_no_final_has_been_logged(self, tenant_a, principal_a):
        mb_id = "del_reportable_none"
        self._terminal_buy_with_webhook(tenant_a, principal_a, mb_id)

        with get_db_session() as session:
            reportable = MediaBuyRepository.get_reportable_for_delivery(
                session, serving_statuses=[], terminal_statuses=["completed"]
            )
        assert mb_id in {mb.media_buy_id for mb in reportable}

    def test_excluded_once_a_successful_final_is_logged(self, tenant_a, principal_a):
        mb_id = "del_reportable_final_done"
        self._terminal_buy_with_webhook(tenant_a, principal_a, mb_id)
        ...
        assert mb_id not in {mb.media_buy_id for mb in reportable}

    def test_still_selected_when_only_a_prior_scheduled_send_succeeded(self, tenant_a, principal_a):
        ...
        mb_id = "del_reportable_scheduled_only"
        self._terminal_buy_with_webhook(tenant_a, principal_a, mb_id)
