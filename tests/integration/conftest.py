...
from tests.integration.migration_helpers import isolated_postgres_db
...
"""
    ...
    def _server_logs() -> tuple[str, str]:
        """Read the spooled subprocess logs without bounded-pipe deadlocks."""
        output: list[str] = []
        for log in (stdout_log, stderr_log):
            log.flush()
            log.seek(0)
            raw = log.read()
            output.append(raw.decode() if raw else "N/A")
        return output[0], output[1]

    def _close_server_logs() -> None:
        stdout_log.close()
        stderr_log.close()
...
def migration_db():
    ...
    Module-scoped: tests in the same file share one cumulative, forward-only-
    migrated database. Fine for verifying schema/DDL state (idempotent if
    already applied), but NOT safe for asserting a one-time DATA transform
    fired -- a migration's UPDATE only runs once, at its own transition, so a
    later test's freshly-seeded rows will NOT be touched by a migration that
    already ran earlier in the module. Use migration_db_fresh for those.
    """
    yield from isolated_postgres_db()


@pytest.fixture
def migration_db_fresh():
    """Function-scoped isolated PostgreSQL database for migration tests that
    need TRUE per-test independence.

    Unlike migration_db (module-scoped, shared across a file), each test using
    this fixture gets its own fresh database -- required when a test asserts
    that a one-time DATA migration's UPDATE actually fired for its own seeded
    rows: on a shared, already-migrated database, "upgrade to the revision
    before mine" is a no-op if some other test already moved past it, so
    freshly-seeded rows would silently never observe that migration's effect.
    """
    yield from isolated_postgres_db()
