...
The row-touching helpers (``seed_tenant``, ``column_exists``, ``get_column_info``)
live in ``tests/helpers/migration_helpers.py`` instead — that directory is inside
the repository-pattern guard's scan set, and shared test substrate belongs where a
guard can see it.
"""
