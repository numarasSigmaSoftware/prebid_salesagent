...
_RESPONSE_MODEL_REGISTRY: list[_RegistryRow] = [
    _RegistryRow(
        schema_ref="protocol/get-adcp-capabilities-response.json",
...
def _synthesize_sample(arm: dict[str, Any], schema_ref: str, model: type | None = None) -> dict[str, Any]:
    ...
    Array required fields with no ``minItems`` → empty list (valid + minimal). An
    array with ``minItems`` → the item schema's own enum, repeated to the minimum
    count (RAISES if the item schema has no enum to derive from). Enums and
    ``$ref``\\ s to enum schemas → a real member. Other types → generate_example_value.
    ...
    for fname in set(arm.get("required", [])) - _VERSION_FIELDS:
        ...
        if spec.get("type") == "array":
            ...
            if item_enum:
                sample[fname] = [item_enum[0]] * min_items
                continue
            raise _CannotSynthesize(
                f"required field {fname!r} of {schema_ref} has minItems={min_items} but its "
                "items have no enum to derive a minimal valid element from — needs an explicit "
                "sample_override"
            )
        try:
...
class TestSampleSynthesisFailsLoud:
    ...
    def test_top_level_required_array_keeps_the_minimal_list_rule(self):
        """RATIFIED BOUNDARY: a top-level required array synthesizes to a minimal valid list.

        ``_synthesize_sample`` short-circuits every required array before
        ``generate_example_value`` is reached: with no ``minItems``, ``[]`` is a
        spec-VALID derived instance (the docstring's original "empty list" rule).
        A field pinned with ``minItems`` (e.g. ``protocol/get-adcp-capabilities-
        response.json``'s ``supported_protocols``, pinned 3.1.1) is no longer
        satisfied by ``[]`` — ``_synthesize_sample`` instead derives the minimal
        valid list from the item schema's own ``enum``, when one exists, and
        RAISES rather than guess when it does not (forcing an explicit
        ``sample_override`` for that row instead).
        ...
        for row in _RESPONSE_MODEL_REGISTRY:
            ...
            for fname in sorted(set(arm.get("required", [])) - _VERSION_FIELDS):
                ...
                if row.sample_override is not None:
                    continue
                min_items = spec.get("minItems", 0)
                sample_value = _synthesize_sample(arm, row.schema_ref, row.model)[fname]
                if not min_items:
                    assert sample_value == []
                else:
                    assert len(sample_value) >= min_items, (
                        f"{row.schema_ref} requires {fname!r} to have at least {min_items} "
                        f"item(s); _synthesize_sample produced {sample_value!r}"
                    )
                    item_enum = spec.get("items", {}).get("enum")
                    assert item_enum is not None and all(v in item_enum for v in sample_value), (
                        f"{row.schema_ref}'s {fname!r} sample {sample_value!r} is not derived "
                        "from the item schema's own enum"
                    )
