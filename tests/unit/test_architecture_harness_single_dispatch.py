...
_KNOWN_DELIVER_OVERRIDES: set[tuple[str, str, str]] = {
    ...
    # UC-006 v3.1.1 key harness: routes A2A through the REAL _run_a2a_handler
    # (genuine Task framing) instead of the parent's build_rest_body-normalized
    # path, so it cannot delegate to CreativeSyncEnv.deliver_a2a either.
    ("tests/harness/creative_sync.py", "CreativeSyncIdempotencyWireEnv", "deliver_a2a"),
    ...
    # Supplies a fresh idempotency_key default (when the caller passed flat
    # kwargs, no typed req) before delegating to the base's declarative
    # MCP_TOOL/A2A_SKILL dispatch — mirrors build_rest_body's identical
    # pre-processing need for REST on the same env.
    ...
    ("tests/harness/test_harness_base.py", "_RawPathEnv", "deliver_a2a"),
    ("tests/harness/test_harness_base.py", "_StashedEnv", "deliver_a2a"),
    ...
    # Test doubles: select the tool/skill/response type from constructor state
    # (self._skill / self._response_type), which no class attribute can express.
    ("tests/integration/test_read_idempotency_transport_persistence.py", "_TaskListEnv", "deliver_mcp"),
    ("tests/integration/test_read_idempotency_transport_persistence.py", "_CrossToolReadEnv", "deliver_mcp"),
    ("tests/integration/test_read_idempotency_transport_persistence.py", "_CrossToolReadEnv", "deliver_a2a"),
}
