...
_KNOWN_DELIVER_OVERRIDES: set[tuple[str, str, str]] = {
    ...
    # The two A2A doubles raise from ``deliver_a2a`` — the seam
    # ``A2ADispatcher.dispatch`` actually calls — to grade the raw-path vs
    # stashed-wire split; overriding ``call_a2a`` instead would never execute.
