...
* ``AdapterCapabilities.supported_pricing_models`` — the field
  ``GET /api/adapters/<type>/capabilities`` puts on the wire
...
They had already drifted. GAM declared ``None`` on the capabilities side, which the admin
UI reads as "all pricing models supported", while refusing everything outside
``{cpm, vcpm, cpc, flat_rate}``. The capabilities field is now derived from the frozenset
by ``AdServerAdapter.__init_subclass__``, leaving one declaration per adapter.
...
def served_pricing_models(adapter_type: str) -> list[str] | None:
    """Return what the capabilities endpoint serves for ``adapter_type``.

    Mirrors the body of ``get_adapter_capabilities`` in
    ``src/admin/blueprints/adapters.py``: the route jsonifies ``asdict()`` of the
    adapter's declared capabilities, or ``{}`` when the adapter declares none.
    """
...
class TestGamReportsWhatItEnforces:
    ...
    def test_gam_is_a_constrained_adapter(self):
        ...
        This is the assertion the old code failed: GAM served ``None``, which the admin
        UI presents as every pricing model supported.
        """
    ...
    def test_validator_accepts_every_model_gam_reports(self, gam_adapter):
        """Every model on the wire is actually buyable."""
        served = served_pricing_models("google_ad_manager")
