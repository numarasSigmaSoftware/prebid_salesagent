...
class TestSandboxIsDeclared:
    """A seller that implements sandbox but never declares it has not shipped it.

    AdCP 3.1.1 media-buy/advanced-topics/sandbox.mdx §Capabilities discovery: "Sellers
    declare sandbox support in `get_adcp_capabilities`" and "If `account.sandbox` is not
    declared or is `false`, the seller does not support sandbox." A conforming buyer
    checks this BEFORE using the mode, so routing sandbox accounts to the simulator
    correctly while staying silent here leaves the feature undiscoverable — the buyer
    never asks for the mode that was made safe.
    """

    @staticmethod
    def _response_for_tenant():
        from unittest.mock import MagicMock

        from src.core.tools.capabilities import _get_adcp_capabilities_impl

        identity = MagicMock()
        identity.principal_id = "p_cap"
        identity.tenant = {
            "tenant_id": "t_cap",
            "name": "Cap Tenant",
            "supported_billing": ["agent", "not-a-real-party"],
        }
        return _get_adcp_capabilities_impl(None, identity)

    def test_account_block_declares_sandbox(self):
        account = self._response_for_tenant().account
        assert account is not None, (
            "capabilities carry no `account` block, so a conforming buyer concludes this "
            "seller does not support sandbox and never uses the mode"
        )
        assert account.sandbox is True, (
            f"account.sandbox is {account.sandbox!r}; the spec treats absent or false as "
            "'not supported', which is what this seller would be advertising"
        )

    def test_billing_is_carried_from_the_tenant_not_invented(self):
        """supported_billing is required by the schema and is a per-tenant column.

        Declaring sandbox must not be bought with a fabricated billing claim — a wrong
        capability is worse than an absent one, since the buyer acts on it.
        """
        account = self._response_for_tenant().account
        assert [b.value for b in account.supported_billing] == ["agent"], (
            "unknown billing strings must be dropped, not coerced: the column is free-form and "
            "predates the enum, so one unrecognised value would otherwise 500 capabilities discovery"
        )

    def test_tenantless_response_omits_the_block_rather_than_misstating_it(self):
        """Without a tenant the only available billing value is an empty list, which
        reads as "supports no billing party at all" — a false statement rather than an
        absent one. Omission is the honest encoding, and a buyer with no tenant context
        cannot act on either."""
        from src.core.tools.capabilities import _get_adcp_capabilities_impl

        assert _get_adcp_capabilities_impl(None, None).account is None
