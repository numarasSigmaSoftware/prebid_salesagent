...
class TestRevisionNumericStringValidationParity:
    ...
    def test_shared_boundary_rejects_numeric_string(self):
        """The shared request boundary rejects it as a typed validation error.

        The concrete wire code is whatever ``adcp_validation_boundary`` emits for
        every request-schema failure on this branch — pinned per transport in
        ...
        with pytest.raises(AdCPValidationError, match="revision"):
