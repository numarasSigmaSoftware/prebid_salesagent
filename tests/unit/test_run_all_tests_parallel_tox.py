...
# A real `tox -p -e <suites>` run leaves `.tox/<suite>.json` on the host via
# the compose bind mount -- nothing else in this stub execs into a container
# to produce them. Fabricate a minimal valid report per suite named after
# `-e` so the later, unstubbed collect_json_reports() step sees the same
# healthy end state a real run would; this test's subject is the tox
# invocation SHAPE, not report-collection failure handling.
args=("$@")
for ((i = 0; i < ${#args[@]}; i++)); do
    if [ "${args[$i]}" = "-e" ] && [ -n "${args[$((i + 1))]:-}" ]; then
        mkdir -p .tox
        IFS=',' read -r -a _suites <<< "${args[$((i + 1))]}"
        for _suite in "${_suites[@]}"; do
            printf '{"summary": {"passed": 1, "failed": 0, "error": 0}}' > ".tox/${_suite}.json"
        done
    fi
done
...
def _run_with_stubbed_docker(tmp_path: Path) -> tuple[subprocess.CompletedProcess, Path]:
    ...
    shutil.copy2(
        _REPO_ROOT / "scripts" / "check_truncated_reports.py",
        workdir / "scripts" / "check_truncated_reports.py",
    )
