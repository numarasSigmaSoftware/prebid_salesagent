...
class TestBlobLogContext:
    ...
    def test_crlf_in_ids_is_neutralized(self):
        ...
        # Every id routes through log_safe here, so the suffix stays a single line. Reddens if
        # the log_safe wrap is removed from any slot.
        #
        # log_safe ESCAPES rather than strips: it delegates to scrub_control_chars, the one home
        # for the control-char defense, which neutralizes every forge-capable character rather
        # than only the CR/LF this test was originally written against. Escaping is the stronger
        # treatment AND keeps the id readable for debugging, so the expected value is the escaped
        # form, not a silently truncated one. The single-line property below is what the CodeQL
        # finding actually requires, and it holds either way.
