...
transaction BEFORE any guarded mutation, and is flipped to
``status='completed'`` (envelope populated) after successful work (atomically
with the domain write where the tool has a single unit of work). The existing
unique index on (tenant, principal, account, key)
