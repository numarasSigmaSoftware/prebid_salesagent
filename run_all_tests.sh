...
record_gate_failure() {
    ...
    # later gate failure (report collection or security audit) fail the run.
    if [ "$RC" -eq 0 ]; then
        RC="${1:-1}"
    fi
}
...
collect_json_reports() {
    ...
    # Re-create the host destination at the point of use: a suite may remove
    # ignored/generated directories while exercising cleanup paths, and this
    # copy once ate a real failure silently (a full 23-minute run finished
    # clean with .tox/*.json all present, but test-results/ never got
    # populated and nothing said why). Idempotent, cheap insurance.
    mkdir -p "$RESULTS_DIR"

    # Copy ONLY the suites THIS invocation ran, never `.tox/*.json` wholesale.
    # `.tox` used to be the tox_data NAMED VOLUME, destroyed by the cleanup
    # trap's `down -v` every run, so a wholesale copy could not pick up
    # anything stale. Removing that volume (so the non-root runner could
    # write `.tox/<env>`) made `.tox` an ordinary bind-mounted directory that
    # PERSISTS between runs -- and silently turned a wholesale copy into a
    # stale-report generator: a suite this run never executed still
    # contributes its last report, and downstream tooling renders it as
    # freshly measured.
    #
    ...
    # consecutive runs published a storyboard.json from hours earlier, and
    # the numbers were read as this run's until an SDK version inside the
    # report contradicted the SDK that was actually installed.
    #
    # Record WHICH suites this invocation ran, next to their reports.
    # Consumers cannot infer it: report timestamps do not separate "stale"
    # from "ran early in a long serial run" (measured: a genuine unit report
    # was 16 min behind the newest, a stale storyboard one 28 min --
    # overlapping bands, so any threshold misfires both ways). An explicit
    # manifest is exact.
    printf '%s\n' "$SUITES" > "$RESULTS_DIR/.suites"

    # A missing report for a suite that DID run is a gate failure, not a
    # warning: it means the suite died before writing one, which is exactly
    # when its absence most needs to be loud.
    for suite in ${SUITES//,/ }; do
        if [ -f ".tox/${suite}.json" ]; then
            if ! cp ".tox/${suite}.json" "$RESULTS_DIR/"; then
                echo "ERROR: failed to copy .tox/${suite}.json into $RESULTS_DIR/ -- see error above." >&2
                collection_rc=1
            fi
        else
            echo "ERROR: no JSON report for suite: ${suite}" >&2
